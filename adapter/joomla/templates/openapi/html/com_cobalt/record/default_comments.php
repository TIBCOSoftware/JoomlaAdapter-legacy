	...
	<form method="post" name="adminForm" id="adminForm" class="form-validate" enctype="multipart/form-data">
		<?php if(count($this->comments)):?>
			<?php foreach ($this->comments AS $comment):?>
                ...
                <?php
                    $renderSubComments = function ($c, $ctxt) use (&$renderSubComments) {
                        if(!empty($c->sub_comments)) {
                    		foreach ($c->sub_comments AS $sub_comment) {
                                $ctxt->comment = $sub_comment;
                    			echo $ctxt->loadTemplate('comments_'.$ctxt->type->params->get('properties.tmpl_comment', 'default'));
                                $renderSubComments($sub_comment, $ctxt);
                            }
                        }
                    }
                ?>
                <?php $renderSubComments($comment, $this);?>
			<?php endforeach;?>
