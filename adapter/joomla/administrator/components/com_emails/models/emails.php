...
/**
 * Methods supporting a list of emails emails.
 *
 * @package     Joomla.Administrator
 * @subpackage  com_emails
 * @since       1.6
 */
class EmailsModelEmails extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Load the parameters.
		$params = JComponentHelper::getParams('com_emails');
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__email_templates').' AS a');
		...
		if (!empty($search))
		{
			if (stripos($search, 'id:') === 0)
			{
				...
				$query->where(
					'('.$db->quoteName('subject').' LIKE '.$search .
