...
class CobaltViewRecords extends JViewLegacy
{
	...
	function display($tpl = NULL)
	{
		...
		if($this->section->id == 5)
		{
        $db_application = JFactory::getDbo(); 
        ...
        field_value in (SELECT field_value from #__js_res_record_values
                ...
                WHERE field_id=77 and type_id=8 and section_id=4 and field_value =" . $user->get('id') . "))))
                        OR id in (SELECT id FROM #__js_res_subscribe WHERE user_id = " . $user->get('id') . " AND `type` = 'record' AND section_id = 5)
                GROUP BY id ORDER BY ".substr($this->get('State')->get('list.ordering'),2)." ".$this->get('State')->get('list.direction');
                $db_application->setQuery($sql);

        $this->items = $db_application->loadObjectList();
		}else{
	...
	private function _prepareDocument()
	{
		...
		if(!$app->input->get('user_id') && !$app->input->get('view_what'))
		{
			...
			/*
			 * uncomment this line to solve the security issue asg-7584:user, product and plan details of one organization
			 * are visible to user from another organization in a link tag which contains rel="canonical"
			 */
