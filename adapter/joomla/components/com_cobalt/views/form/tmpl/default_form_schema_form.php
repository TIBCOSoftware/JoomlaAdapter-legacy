	...
	<?php if($this->multirating):?>
		<div class="control-group odd<?php echo $k = 1 - $k ?>">
			<label id="jform_multirating-lbl" class="control-label" for="jform_multirating" >
				<?php echo JText::_(strip_tags($this->form->getLabel('multirating')));?></label>
			<div class="controls">
...
<script>
    jQuery(function(){
        var schema = jQuery('textarea[name="jform[fields][216]"]');
        schema.css('height','300px');
        decodeXML();

        function encodeXML() {
            schema.val(DeveloperPortal.java7EncodeURIComponent(schema.val()));
        }
        function decodeXML() {
            schema.val(DeveloperPortal.java7DecodeURIComponent(schema.val()));
        }
