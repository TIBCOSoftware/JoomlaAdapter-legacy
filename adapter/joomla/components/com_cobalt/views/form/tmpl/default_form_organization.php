...
if(strpos($path,'userprofile') || strpos($path, 'dashboard')){
  if(!(in_array(7, $auth_group_ids) || in_array(8, $auth_group_ids) || in_array($org_admin_group_id, $auth_group_ids))){
...
<?php 	$comEmail = JComponentHelper::getComponent('com_emails');
...
<script type="text/javascript">
var oldThreshold = jQuery('input[name="jform[fields][130]"]').val();
Joomla.beforesubmitform = function(fCallback, fErrorback) {
  var newThreshold = jQuery('input[name="jform[fields][130]"]').val();
