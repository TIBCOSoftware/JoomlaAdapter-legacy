...
if(JComponentHelper::getParams('com_emails')->get('enable_deleting_objects') == 1) {
    $tasks_to_hide = DeveloperPortalApi::isReferencedByDownstreamSubs($this->item->id, "mapping") ? array(DeveloperPortalApi::TASK_DELETE) : array();
} else {
...
<script>
    jQuery('div.mapping-display').text(function(index, text){
        return DeveloperPortal.java7DecodeURIComponent(text);
    });
</script>
