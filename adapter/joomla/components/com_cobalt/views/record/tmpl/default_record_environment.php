...
if(JComponentHelper::getParams('com_emails')->get('enable_deleting_objects') == 1) {
  $tasks_to_hide = DeveloperPortalApi::isReferencedByDownstreamSubs($this->item->id, "environment") ? array(DeveloperPortalApi::TASK_DELETE) : array();
} else {
...
<?php
	/**
	 * Get the config UI URL from component
	 */
	$asg_config = JComponentHelper::getComponent('com_emails');
	$asg_configui_url = $asg_config->params->get("configui_url");

	if($asg_configui_url == 'http://')
	{
		$asg_configui_url = "";
	}
	if ($asg_configui_url && substr($asg_configui_url, -1) != "/") {
		$asg_configui_url = $asg_configui_url."/";
	}

?>
<script type="text/javascript">

jQuery("#configUIbutton").click(toggleIframe);

function toggleIframe(bool){
	 var configui_url = "<?php echo $asg_configui_url;?>";
	 		 configui_url = configui_url || GLOBAL_CONTEXT_PATH;
   var environmentTitle= "<?php echo urlencode($item->title)?>";
   var bool = jQuery(this).hasClass('iframe_active');
   var src = bool? configui_url + 'ConfigUI/'+'?='+environmentTitle:"about:blank";
